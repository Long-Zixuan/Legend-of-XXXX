...
public class ProjectorMovement : MonoBehaviour
{
    ...
    public void Draw2CharacterLogic()
    {
        if (isActive)
        {
    ...
    private void Update()
    {
        ...
        if (!CanMove(axis))
        {
            Debug.Log("You can't Keep Moving");
            return;
        }
    ...
    bool CanMove(float axis)
    {
        Ray ray = new Ray(transform.position, -transform.right * axis);
        RaycastHit hitInfo;

        if (Physics.Raycast(ray, out hitInfo,rayDistance))
        {
            Debug.Log("Hit Obj Name:" + hitInfo.collider.gameObject.name);
            Debug.Log("Hit Obj Tag:" + hitInfo.collider.gameObject.tag);
